            ...
            <ul className="aap">
                {props.names.map((x) => {
                  return (
                    <li 
                    onClick={() => 
                      props.checkValue() && props.boxes[props.names.indexOf(x)] ? 
                      props.handleFilterChangeSelection("deselect") : 
                      props.handleFilterChange(props.names.indexOf(x))}> 
                    <input 
                    type="checkbox" 
                    id="filterbox0" 
                    name="boxes" 
                    readOnly={true}
                    checked={props.boxes[props.names.indexOf(x)]}
                    /> {x}
                </li>
                  )
                })}
